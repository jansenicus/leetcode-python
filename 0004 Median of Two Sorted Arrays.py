class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        L, R = 0, n * 2                     # shortest imaginary placeholder named LOVER where they need to find V
                                            ...
                                            # keep on searching as long as L <= R
                                            # stop when L > R as we got the max(L1, L2) or equivalently min(R1,R2)
        
        while L <= R:
        
            v2 = (L + R) // 2               # v2 is the midpoint of L and R
            v1 = m + n - v2                 # v1 is the total length of m and n minus v2
            ...
            L1 < R1 == True                                 # it is guaranteed that every L1 < R1 due to sorted array
            L2 < R2 == True                                 # it is guaranteed that every L2 < R2 due to sorted array

            if L1 > R2:                                     # when there are more smaller numbers on L1 than on R2
                L = v2 + 1                                  # shift v1 toward the smaller -> shift v2 toward the greater
                                                                            
            elif L2 > R1:                                   # when there are more smaller numbers on L2 than on R1
                R = v2 - 1                                  # shift v2 toward the smaller
